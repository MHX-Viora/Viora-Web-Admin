...
const emptyPack: SaveStickerPack = { name: '', description: null, thumbnailUrl: '', price: 0, isFeatured: false, isActive: true, sortOrder: 0, availableFrom: null, availableUntil: null };

export function StickerPacksPage() {
  const client = useQueryClient(); const [selectedId, setSelectedId] = useState(''); const [draft, setDraft] = useState(emptyPack); const [thumbnailFile, setThumbnailFile] = useState<File | null>(null); const [stickerName, setStickerName] = useState(''); const [stickerUrl, setStickerUrl] = useState('');
  ...
  }, onSuccess: async (pack) => { toast.success(selectedId ? 'Đã cập nhật bộ nhãn dán' : 'Đã tạo bộ nhãn dán'); setSelectedId(pack.id); setThumbnailFile(null); setDraft({ name: pack.name, description: pack.description, thumbnailUrl: pack.thumbnailUrl, price: pack.price, isFeatured: pack.isFeatured, isActive: pack.isActive, sortOrder: pack.sortOrder, availableFrom: pack.availableFrom, availableUntil: pack.availableUntil }); await refresh(); }, onError: (error) => toast.error(getErrorMessage(error)) });
  ...
  return <section><PageHeader eyebrow="Nội dung chat" title="Nhãn dán" description="Quản lý bộ nhãn dán, ảnh CDN, giá và trạng thái mà không xoá lịch sử chat." actions={<button className="btn secondary" onClick={() => void refresh()} type="button"><RefreshCw size={16} /> Làm mới</button>} />
    <div className="admin-sticker-grid"><div>{packs.isLoading ? <Loading /> : null}{packs.isError ? <ErrorView message={getErrorMessage(packs.error)} onRetry={() => void packs.refetch()} /> : null}{packs.data ? <DataTable columns={columns} items={packs.data} onRowClick={(item) => { setSelectedId(item.id); setThumbnailFile(null); setDraft({ name: item.name, description: item.description, thumbnailUrl: item.thumbnailUrl, price: item.price, isFeatured: item.isFeatured, isActive: item.isActive, sortOrder: item.sortOrder, availableFrom: item.availableFrom, availableUntil: item.availableUntil }); }} /> : null}</div>
      <aside className="sticker-editor"><div className="section-heading"><h2>{selectedId ? 'Sửa bộ nhãn dán' : 'Tạo bộ nhãn dán'}</h2>{selectedId ? <button className="btn ghost" onClick={() => { setSelectedId(''); setThumbnailFile(null); setDraft(emptyPack); }} type="button">Tạo mới</button> : null}</div><label>Tên<input value={draft.name} onChange={(e) => setDraft({ ...draft, name: e.target.value })} /></label><div className="sticker-thumbnail-field"><span>Ảnh đại diện</span><label className="btn secondary"><ImagePlus size={16} /> Chọn ảnh từ máy<input accept="image/jpeg,image/png,image/webp" hidden type="file" onChange={(event) => chooseThumbnail(event.target.files?.[0])} /></label><small>{thumbnailFile?.name ?? (selectedId ? 'Giữ ảnh hiện tại nếu không chọn ảnh mới' : 'JPEG, PNG hoặc WebP · tối đa 5 MB')}</small>{selectedId && draft.thumbnailUrl ? <img alt={`Ảnh đại diện ${draft.name}`} src={draft.thumbnailUrl} /> : null}</div><label>Giá xu<input min="0" type="number" value={draft.price} onChange={(e) => setDraft({ ...draft, price: Number(e.target.value) })} /></label><label>Thứ tự<input type="number" value={draft.sortOrder} onChange={(e) => setDraft({ ...draft, sortOrder: Number(e.target.value) })} /></label><label>Mở từ<input type="datetime-local" value={draft.availableFrom?.slice(0, 16) ?? ''} onChange={(e) => setDraft({ ...draft, availableFrom: e.target.value ? new Date(e.target.value).toISOString() : null })} /></label><label>Đến<input type="datetime-local" value={draft.availableUntil?.slice(0, 16) ?? ''} onChange={(e) => setDraft({ ...draft, availableUntil: e.target.value ? new Date(e.target.value).toISOString() : null })} /></label><label className="check"><input checked={draft.isFeatured} type="checkbox" onChange={(e) => setDraft({ ...draft, isFeatured: e.target.checked })} /> Nổi bật</label><label className="check"><input checked={draft.isActive} type="checkbox" onChange={(e) => setDraft({ ...draft, isActive: e.target.checked })} /> Hiển thị</label><button className="btn primary" disabled={!draft.name || (!selectedId && !thumbnailFile) || savePack.isPending} onClick={() => savePack.mutate()} type="button"><StickerIcon size={16} /> {selectedId ? 'Lưu thay đổi' : 'Tạo bộ'}</button></aside></div>
