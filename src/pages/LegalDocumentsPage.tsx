...
import { createLegalDocument, createLegalVersion, deleteLegalDocument, getLegalDocument, getLegalDocuments, publishLegalDocument, type LegalInput } from '../services/admin-legal.service';
...
export function LegalDocumentsPage() {
  ...
  const refresh = () => client.invalidateQueries({ queryKey: ['legal-documents'] });
  ...
  return <>
    ...
    {query.isLoading ? <p>Đang tải…</p> : query.isError ? <div role="alert"><p>{getErrorMessage(query.error)}</p><button className="btn" onClick={() => void query.refetch()} type="button">Thử lại</button></div> : !query.data?.length ? <p>Chưa có tài liệu.</p> :
